...
import 'package:bridalglow_desktop/screens/dress_tag_form_screen.dart';

const _kPrimary = Color(0xFFC2778A);
const _kPrimaryLight = Color(0xFFFFF0F3);

class DressTagListScreen extends StatefulWidget {
  const DressTagListScreen({super.key});

  @override
  State<DressTagListScreen> createState() => _DressTagListScreenState();
}

class _DressTagListScreenState extends State<DressTagListScreen> {
  late DressTagProvider _provider;
  final _nameController = TextEditingController();

  SearchResult<DressTag>? _result;
  int _currentPage = 0;
  int _pageSize = 10;
  bool _loading = false;
  ...
  void initState() {
    ...
    WidgetsBinding.instance.addPostFrameCallback((_) async {
      _provider = context.read<DressTagProvider>();
  ...
  Future<void> _openForm({DressTag? tag}) async {
    final saved = await Navigator.push<bool>(
      context,
      MaterialPageRoute(
        builder: (_) => DressTagFormScreen(tag: tag),
      ),
    );
    if (saved == true) await _performSearch();
  }

  Future<void> _confirmDelete(DressTag tag) async {
      ...
      builder: (ctx) => AlertDialog(
        ...
        content: RichText(
          text: TextSpan(
            ...
            children: [
              const TextSpan(text: 'Are you sure you want to delete tag '),
              TextSpan(
                text: '"${tag.name}"',
    ...
    if (confirmed == true && mounted) {
      try {
        ...
        if (mounted) {
          _showSnackBar('Tag "${tag.name}" deleted.', Colors.green);
    ...
    return Row(
      children: [
        Container(
          ...
          child: const Icon(Icons.sell_outlined, color: Colors.white, size: 24),
        ),
        ...
        const Expanded(
          child: Column(
            ...
            children: [
              ...
              Text(
                'Manage tags for filtering and discovery',
  ...
  Widget _buildTable(List<DressTag> items) {
    return Container(
      decoration: BoxDecoration(
        color: Colors.white,
      ...
      child: Column(
        children: [
          ...
          Container(
            ...
            child: Row(
              children: [
                const Icon(Icons.sell_outlined, color: _kPrimary, size: 20),
  ...
  TableRow _buildTableDataRow(DressTag item) {
    return TableRow(
      decoration: BoxDecoration(
  ...
  Widget _buildActions(DressTag item) {
    return Row(
  ...
  Widget _buildEmptyState() {
    return Center(
      child: Column(
        ...
        children: [
          Icon(Icons.sell_outlined, size: 48, color: Colors.grey.shade300),
