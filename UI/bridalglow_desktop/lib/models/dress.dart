...
// ── Enum label maps (int key → display string) ────────────────────────────
...
// ── Enum name → int parsers ────────────────────────────────────────────────
// The API uses JsonStringEnumConverter, so enums arrive as their C# member
// names ("Draft", "OutOfService", "VeryGood", …), not as integers.
// These helpers accept both string and int so the models are robust to any
// future serialisation change.
...
/// Lightweight model for the Dress list view (DressListItemResponse).
/// The [fromJson] method intentionally handles both DressListItemResponse
/// (tagNames: List of Strings) and DressResponse (tags: List of Objects) so
/// that the same provider can be used for insert/update operations which
/// return the full DressResponse from the API.
class DressListItem {
  final int id;
  final String code;
  final String name;
  ...
  factory DressListItem.fromJson(Map<String, dynamic> json) {
    // Handle both DressListItemResponse (tagNames) and DressResponse (tags)
